...
#import library (if not installed yet use pip install)
import os
import platform
import subprocess
import gspread
...
def group_forming():
    ...
    if len(data) <= 2:
        ...
        new_groups_csv = "Coffee_Partner_Lottery_new_groups.csv" #for us to see which groups are made
        all_groups_csv = "Coffee_Partner_Lottery_all_groups.csv" #for us to see which past groups are made and that the groups will not be repeated 
        ...
        #saves formatted output to a file and will pop up for the participants to see in which group they are and what their icebreaker is
        save_output_to_file(output_string, "EspressYoSelf_Groups.txt")

        #safe groups to csv-file (not for the participants but for own use)
